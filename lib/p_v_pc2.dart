...
class PvPCScreen2 extends StatefulWidget {
  
  const PvPCScreen2({Key? key}) : super(key: key);

  @override
  State<PvPCScreen2> createState() => _PvPC2ScreenState();
}

class _PvPC2ScreenState extends State<PvPCScreen2> {
  double x = -50, y = 0, w = 60, h = 80, angulo = 0, radio = 100;
  double x2 = 50, y2 = 0, w2 = 60, h2 = 80, angulo2 = pi, radio2 = 100;
  bool _disposed = false;
  int dirNave = 1,  anguloNave = 1, modRand = 6;
  ...
  mover() {
    if(!_disposed){
      setState(() {
        ...
        if(angulo2>2 && anguloNave==1){
          angulo2 -= 0.3;
        } else{
          anguloNave = -1;
        }
        if(angulo2<4.3 && anguloNave==-1){
          angulo2 +=0.3;
        }else{
          anguloNave=1;
        }
